...
class DexGRAB(Dataset):
    ...
    _test_split = ['cup', 'cup', 'cup', 'torusmedium']

    # body category maybe a bit out-of-distribution, but the following include the body category
    # _all_split = ['cylindersmall', 'cylindermedium', 'toruslarge', 'camera', 'train', 'mug', 'knife', 'binoculars', 'spheresmall', 
    #               'airplane', 'torusmedium', 'rubberduck', 'apple', 'cubesmall', 'wristwatch', 'cylinderlarge', 'flute', 'stamp', 
    #               'scissors', 'bowl', 'pyramidlarge', 'toothbrush', 'body', 'cubemedium', 'teapot', 'duck', 'gamecontroller', 'hammer', 
    #               'flashlight', 'waterbottle', 'torussmall', 'headphones', 'mouse', 'cubemiddle', 'stapler', 'elephant', 'piggybank', 
    #               'alarmclock', 'cubelarge', 'cup', 'wineglass', 'lightbulb', 'watch', 'phone', 'eyeglasses', 'spherelarge', 'spheremedium', 
    #               'toothpaste', 'doorknob', 'stanfordbunny', 'hand', 'coffeemug', 'pyramidmedium', 'fryingpan', 'table', 'pyramidsmall', 'banana']
    # _test_split = ['torusmedium', 'train', 'toothpaste', 'eyeglasses', 'cylindermedium', 'camera', 'pyramidmedium', 'body', 'gamecontroller', 
    #                'duck', 'pyramidsmall', 'torussmall', 'table', 'mouse', 'wristwatch', 'spherelarge', 'stapler', 'cylinderlarge']
    ...
    # translation 
    # _global_trans_mean = torch.tensor([-0.0316473, 0.10764614, -0.32223194])
    # _global_trans_std = torch.tensor([0.14903177, 0.09685536, 0.11557627])

    # _global_trans_lower = _global_trans_mean - 2 * _global_trans_std
    # _global_trans_upper = _global_trans_mean + 2 * _global_trans_std
    ...
    def __init__(self, cfg: DictConfig, phase: str, slurm: bool, case_only: bool=False, **kwargs: Dict) -> None:
        ...
        self.data_dir = "/inspurfs/group/mayuexin/datasets/DexGRAB"
        self.scene_path = os.path.join('/inspurfs/group/mayuexin/datasets/DexGRAB', 'object_pcds_nors.pkl')
    ...
    def _pre_load_data(self, case_only: bool) -> None:
        ...
        grasp_dataset_file = os.path.join(self.data_dir, '0.01filter_shadowhand_downsample.pt')
        grasp_dataset = torch.load(grasp_dataset_file)
        # grasp_dataset_file = os.path.join(self.data_dir, 'dexgrab_tight.pkl')
        self.scene_pcds = pickle.load(open(self.scene_path, 'rb'))
        self.dataset_info = grasp_dataset['info']
        if self.use_llm:
            # Getting descriptions from LLM
            scene_text_file = "/inspurfs/group/mayuexin/datasets/dex_hand_preprocessed/DexGRAB_gpt4o_mini.json"
