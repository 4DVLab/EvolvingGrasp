...
class DexGraspNet(Dataset):
    ...
    # read json
    input_file = "/inspurfs/group/mayuexin/datasets/DexGraspNet/grasp.json"
    _train_split, _test_split, _all_split = load_from_json(input_file)
    _test_split = _test_split[:50]
    # _test_split = ['core-bottle-10dff3c43200a7a7119862dbccbaa609', 'ddg-bigbird_band_aid_sheer_strips', 
    # 'mujoco-5_HTP', 'sem-Bear-3695cb6bc925dfa1f6a5b0146bf3030', 'sem-Book-fe3ccb1376a4b6b34e309f47b24dd64',
    # 'sem-Bottle-10d3d5961e00b2133ff038bc77759685', 'core-bowl-e30e5cbc54a62b023c143af07c12991a', 'core-camera-ec28a64bdf9501161bfbba8a5defb02', 
    # 'core-can-295be2a01cb9f29b716714dd1fd945b7', 'core-cellphone-1105c21040f11b4aec5c418afd946fad']
    # _test_split = ['core-camera-ec28a64bdf9501161bfbba8a5defb02', 'core-camera-ec28a64bdf9501161bfbba8a5defb02', 'core-camera-ec28a64bdf9501161bfbba8a5defb02']
    ...
    def __init__(self, cfg: DictConfig, phase: str, slurm: bool, case_only: bool=False, **kwargs: Dict) -> None:
        ...
        self.data_dir = os.path.join('/inspurfs/group/mayuexin/datasets/DexGraspNet')
        self.scene_path = os.path.join('/inspurfs/group/mayuexin/datasets/DexGraspNet', 'point_cloud_data_10240_nors.pkl')
    ...
    def _pre_load_data(self, case_only: bool) -> None:
        ...
        self.frames = []
        self.scene_pcds = {}
        # grasp_dataset_file = os.path.join(self.data_dir, 'dexgraspnet_shadowhand_downsample.pkl')
        # with open(grasp_dataset_file, "rb") as pklf:
        #     grasp_dataset = pickle.load(pklf)
        
        # grasp_dataset = torch.load(os.path.join(self.data_dir, 'dexgraspnet_shadowhand_downsample.pt' if self.is_downsample else 'dexgraspnet_shadowhand.pt'))
        grasp_dataset = torch.load(os.path.join(self.data_dir, '0.01filter_shadowhand_downsample.pt' if self.is_downsample else 'dexgraspnet_shadowhand_downsample.pt'))
        # grasp_dataset = torch.load(os.path.join(self.data_dir, '0.01filter_shadowhand_downsample.pt' if self.is_downsample else 'dexgraspnet_shadowhand.pt'))
        ...
        if self.use_llm:
            ...
            scene_text_file = "/inspurfs/group/mayuexin/datasets/dex_hand_preprocessed/DexGraspnet_gpt4o_mini.json"
