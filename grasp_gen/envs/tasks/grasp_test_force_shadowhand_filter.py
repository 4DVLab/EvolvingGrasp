    ...
    def _load_obj_asset(self):
        ...
        with tqdm(total=used_len) as pbar:
            ...
            if self.object_name is None:
                ...
                ### grasp_anyting
                # object_mesh_path = os.path.join('/inspurfs/group/mayuexin/datasets/scaled_PUnetdata/mesh', f'{self.object_name}.obj')
                # object_urdf_path = os.path.join('/inspurfs/group/mayuexin/datasets/scaled_PUnetdata/mesh', f'{self.object_name}.urdf')
                ...
                # scale_str = self.scale.replace("_", ".")
                # # Convert to float
                # scale = float(scale_str)
                # if scale < 3.0:
                #     base_path = os.path.normpath(f'/inspurfs/group/mayuexin/datasets/DexGraspNet/scaled_obj_urdf/scale_{self.scale}')
                # else:
                #     base_path = os.path.normpath(f'/inspurfs/group/mayuexin/datasets/UniDexGrasp/DFCData/scaled_obj_urdf/scale_{self.scale}')
                # object_mesh_path = os.path.join(base_path, f'{self.object_name}.obj')
                # object_urdf_path = os.path.join(base_path, f'{self.object_name}.urdf')
