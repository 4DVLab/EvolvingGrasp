...
class GraspGenURVisualizer():
    ...
    def visualize(
        ...
        if cfg.task.name == 'grasp_gen_ur':
            ...
            save_dir = '/inspurfs/group/mayuexin/zhuyufei/graps_gen/outputs/visualize_outputs_all'
        # os.makedirs(save_dir, exist_ok=True)
        # save
        ...
        for object_name in dataloader.dataset._test_split:    # use all obj
            ...
            for epoch_obj in range(0, total_epoches_obj):
                model.eval()            ### eval模式
                save_dir_end = os.path.join(save_dir, f'{object_name}', f'{epoch_obj}')
                ...
                rewards_, rewards_ori, _ = evaluation_grasp(cfg=cfg, grasps=res, obj_mesh=obj_mesh, device=device, object_name=object_name)

                '''
                output_trans = apply_transformations(outputs[:, 9:], device,
                                robust_compute_rotation_matrix_from_ortho6d(id_6d_rot).to(torch.float32), outputs[:, :3].to(torch.float32))
                
                disc_test_dict = {
                    'hand_t': trans_normalize(outputs[:, :3].to(torch.float32), device), 
                    'hand_R': normalize_rot_torch(outputs[:, 3:9].to(torch.float32), rot_type='mat'),
                    'hand_param': angle_normalize(output_trans.to(torch.float32), device),
                    'object_pc': object_pc,
                }

                # disc_test_dict = {
                #     'hand_t': normalize_trans_torch(outputs[:, :3].to(torch.float32)), 
                #     'hand_R': normalize_rot_torch(outputs[:, 3:9].to(torch.float32), rot_type='mat'),
                #     'hand_param': normalize_param_torch(output_trans.to(torch.float32)),
                #     'object_pc': object_pc,
                # }
                ...
                pred = torch.sigmoid(disc_out_dict['pred'])
                '''
                # pdb.set_trace()
                    ...
                    for epoch in tqdm(range(0,total_batch_size),
                        ...
                        for j in tqdm(
                            ...
                            with autocast():
                                ...
                                # 
                                total_prob_0 = model.sample_logprob(                                #############
                                    noise_pred_0,
                                    sample_0["timesteps"][j],
                                    sample_0["latents"][j],
                                    # eta=model.eta,
                                ...
                                total_ref_prob_0 = model.sample_logprob(                            #############
                                    noise_ref_pred_0,
                                    sample_0["timesteps"][j],
                                    sample_0["latents"][j],
                                    # eta=model.eta,
...
def create_visualizer_1(cfg: DictConfig) -> nn.Module:
