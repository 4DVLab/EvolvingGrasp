...
class GraspGenURVisualizer():
    ...
    def visualize(
        ...
        if cfg.task.name == 'grasp_gen_ur':
            ...
            # save_dir = '/inspurfs/group/mayuexin/zhuyufei/graps_gen/outputs/w_o_rate_pkl_visualize/visualize_outputs_all'
            save_dir = '/inspurfs/group/mayuexin/zhuyufei/graps_gen/outputs/degraded/visualize_outputs_all'
        save_res_dir = '/inspurfs/group/mayuexin/zhuyufei/graps_gen/outputs/w_o_rate_pkl_visualize/respkl'
        # os.makedirs(save_dir, exist_ok=True)
        # save
        ...
        ref = copy.deepcopy(model.model.eps_model)
        # ref = copy.deepcopy(diffuser.model.eps_model)
        ...
        for object_name in dataloader.dataset._test_split:    # use all obj
            ...
            path = os.path.join(
                cfg.save_RL_dir, 
                f'model_{object_name}.pth'
            )
            save_ckpt(
                model=model, epoch=0, step=0, path=path,
            ...
            for epoch_obj in range(0, total_epoches_obj):
                # diffuser.model.eval()
                # model.model.eval()            ### eval模式
                model.eval()
                save_dir_end = os.path.join(save_dir, f'{object_name}', f'{epoch_obj_i}')
                save_dir_pkl_end = os.path.join(save_res_dir, f'{object_name}', f'{epoch_obj_i}')
                epoch_obj_i = epoch_obj_i + 1
                # os.makedirs(save_dir_end, exist_ok=True)
                os.makedirs(save_dir_pkl_end, exist_ok=True)
                # if global_step == 20:
                #     break
                global_step += 1
                samples = []
                ...
                # latents, log_probs, timesteps = diffuser.sample(data, k=1)
                ...
                ######   grasp_anyting
                # scene_object = scene_id
                # mesh_path = os.path.join('/inspurfs/group/mayuexin/datasets/scaled_PUnetdata/mesh', f'{scene_object}.obj')
                # obj_mesh = trimesh.load(mesh_path)
                ...
                for i in range(outputs.shape[0]):
                    ...
                    if i < 100:
                        ...
                        # save_img_path = os.path.join(save_dir_end, f'{object_name}_sample-{i}.png')
                        fig = go.Figure(data=vis_data)
                        fig.update_layout(
                            scene=dict(
                        ...
                        # fig.write_image(save_img_path)      ## 注意开头的save_dir设置
                        fig.write_html(save_path)
                    
                    '''
                    # 这里有32个生成的pose的可视化，可以在线收集人类的反馈
                    pbar.update(1)
                res['sample_qpos'][object_name] = np.array(outputs.cpu().detach())
                # pdb.set_trace()
                pickle.dump(res, open(os.path.join(save_dir_pkl_end, 'res_diffuser.pkl'), 'wb'))
                # compute rewards using discriminator  or issac gym

                rewards_, _, _, rewards_ori, _ = evaluation_grasp(cfg=cfg, grasps=res, obj_mesh=obj_mesh, device=device, object_name=object_name)
                rewards = torch.where(rewards_, torch.tensor(1, device='cuda:0'), torch.tensor(-1, device='cuda:0'))
                evaluations.append(sum(rewards_ori))

                # rewards = torch.randint(0, 2, (batch_size,)).float().to(device) * 2 - 1
                # '''
                if epoch_obj >= 1:
                    # pdb.set_trace()
                    #  if evaluations[epoch_obj] - evaluations[epoch_obj - 1] >= 6:
                    for param_group in optimizer.param_groups:
                        param_group['lr'] *= 0.1 ** (evaluations[epoch_obj] - evaluations[epoch_obj - 1])
                        if param_group['lr'] > cfg.learning_rate:
                            param_group['lr'] = cfg.learning_rate
                # '''
                # pdb.set_trace()
                ...
                if sum(rewards) > 0 :
                    for param_group in optimizer.param_groups:
                        param_group['lr'] = cfg.lr_decay1
                if sum(rewards) > -8 & sum(rewards) < 1 :
                    for param_group in optimizer.param_groups:
                        param_group['lr'] = 0.1 * cfg.lr_decay2

                if sum(rewards) < -7 :
                    for param_group in optimizer.param_groups:
                        param_group['lr'] = cfg.lr_decay2
                if sum(rewards) > 7 :
                    for param_group in optimizer.param_groups:
                        param_group['lr'] = cfg.lr_decay3
                '''
                ...
                total_epoches = 10
                for _ in range(0, total_epoches):
                            ...
                            with torch.autograd.detect_anomaly():
                                with autocast():
                                    ...
                                    for i in range(len(total_ref_prob_0)):
                                        ...
                                        tmp = tmp + beta*(torch.log(ratio[i]))*rewards[i]
                                    
                                    # tmp = sum(tmp_)
                                    loss = -torch.log(torch.sigmoid(tmp)).mean()                     ################### + or -
...
def create_visualizer_CM(cfg: DictConfig) -> nn.Module:
